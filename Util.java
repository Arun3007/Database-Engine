...
public class Util {
  ...
  public static String [] parseStrings(String s) {
        ...
        try {
            ...
            while ((type = st.nextToken()) != StreamTokenizer.TT_EOF) {
                if (type==StreamTokenizer.TT_WORD) {
                    if (num < 40) words[num++] = st.sval;
                    
                } else if (type==StreamTokenizer.TT_NUMBER) {
                    if (num < 40) words[num++] = (new Integer((int)st.nval)).toString();
    ...
    if (num>0) {
        ...
        System.out.println("Inside Util.ParseStrings words generated are as below \n");
        for (int i=0; i<num; i++) {
            ret[i]=words[i];
